...
public:
  ...
  share_data_(share_data), host_(host), port_(port), io_context_(), ws_(io_context_){
    std::cout << "video stream constructor" << std::endl;
  };

  bool init(){
    gst_init(nullptr, nullptr);
    if(!init_webrtc_stream()){
      return false;
    }
    return true;
  }
  ...
  void stop(){
    std::cout << "stop video stream" << std::endl;
    is_exit_.store(true);
  ...
  ~VideoStream(){
    std::cout << "video stream destructor" << std::endl;
    stop();
...
private:
  bool init_webrtc_stream(){
    ...
    GstElement *rtppay = gst_element_factory_make("rtpvp8pay", "rtppay");
    ...
    if(!appsrc_){
      std::cerr << "Failed to create appsrc" << std::endl;
      return false;
    }
    if(!jpeg_dec){
    ...
    if(!rtppay){
      std::cerr << "Failed to create rtppay" << std::endl;
      return false;
    }
    if(!webrtcbin_){
      std::cerr << "Failed to create webrtcbin" << std::endl;
      return false;
    }

    GstCaps* appsrc_caps = gst_caps_new_simple("image/jpeg",
      "width", G_TYPE_INT, share_data_->get_width(),
      "height", G_TYPE_INT, share_data_->get_height(),
      nullptr);
    g_object_set(appsrc_, "caps", appsrc_caps, nullptr);
    ...
    g_object_set(webrtcbin_, "stun-server", "stun://stun.l.google.com:19302", nullptr);

    gst_bin_add_many(GST_BIN(pipeline_), appsrc_, 
      jpeg_dec, queue, videoconvert, capsfilter, enc, rtppay, 
      webrtcbin_, nullptr);

    if(!gst_element_link_many(appsrc_, jpeg_dec, 
      queue, videoconvert, capsfilter, enc, rtppay, webrtcbin_, nullptr)){
  ...
  void connect_to_signaling_server(){
    std::cout << "start to connect signaling server" << std::endl;
    while(!is_exit_.load() && !running_.load()){
      try{
        ...
        std::cerr << "Error in connect_to_signaling_server: " << e.what() << std::endl;
  ...
  void listen_for_signaling_messages(){
    ...
    try{
      ...
      std::cerr << "Error in listen_for_signaling_messages: " << e.what() << std::endl;
    }catch(...){
      std::cerr << "unknow error" << std::endl;
    }
    std::cout << "listening thread exiting" << std::endl;
  }
    ...
    while(!is_exit_.load()){
      ...
      }
      {
        ...
        GstBuffer* gst_buffer = gst_buffer_new_wrapped_full(
          ...
          share_data_->get_frame_buffer()->start,
          share_data_->get_width() * share_data_->get_height() * 2,
          0,
          share_data_->get_width() * share_data_->get_height() * 2,
          nullptr,
          nullptr

...
public:
  using Ptr = std::shared_ptr<MjpegToYuvConverter>;
  MjpegToYuvConverter(u_int32_t width, u_int32_t height): width_(width), height_(height){
  ...
  bool init(){
    ...
    ret_ = mpp_buffer_get(mpp_packet_group_, &mpp_packet_buffer_, width_ * height_ * 3);
    if (ret_ != MPP_OK) {
      std::cout << "mpp_buffer_get failed, ret = " << ret_ << std::endl;
      return false;
    }
  ...
  bool mppFrame2RGB(const MppFrame frame, uint8_t *data) {
    ...
    rga_set_rect(&dst_info.rect, 0, 0, width, height, width*3, height,
          RK_FORMAT_RGB_888);
  ...
  ~MjpegToYuvConverter(){
    std::cout << "MjpegToYuvConverter destructor" << std::endl;
